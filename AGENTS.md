...
- **`data\.env_key` is an AI-attack-surface.** It stores the plaintext passphrase on disk so the launcher can run non-interactively. If the active nanobot has file-read tools enabled and workspace restrictions do not cover `data/`, the LLM can read this file and decrypt `data\.env.encrypted` itself, defeating the encryption's purpose. The encryption exists to keep the API key out of LLM output. Prefer interactive passphrase entry (`delete data\.env_key`) for full AI-proof guarantee.
