...
- No Python/Node/Git/gh on the host — `nanobot-setup.ps1` fetches portable versions into `bin/`.
...
edit_env.bat          # decrypt .env → notepad → re-encrypt (also shows active provider context)
build-webui-custom.bat # overlay data\webui-src\ onto app\webui\ then build
scripts/
  init_portable.ps1   # dot-sourced by all root .ps1; redirects USERPROFILE/HOME/TEMP to portable paths
  setup/              # install_python, install_git, install_gh, install_nodejs, install_source, install_deps, download, extract, setup_helpers
  ...
  install_webui.ps1     # npm install + build + deploy to site-packages
  sync_webui.ps1        # push data\webui\ drop zone to site-packages (idempotent mtime check)
...
- Check for a newer portable release: `check_update.bat`  (records a baseline SHA in `data\.lockhead [sha]` section on first run; not auto-invoked)
- Build custom webui: `build-webui-custom.bat`  (overlays `data\webui-src\` onto `app\webui\`, then invokes `build-webui.bat`)
...
- **Whitelist `.gitignore`.** The root `.gitignore` starts with `/*` and only re-allows specific paths. Any new top-level file or directory must be added to `.gitignore` with a `!/path` rule, otherwise it will silently not be tracked. Confirmed tracked set: `setup.bat`, `start-chat.bat`, `start-gateway.bat`, `edit_env.bat`, `check_update.bat`, `build-webui.bat`, `build-webui-custom.bat`, `sync-webui.bat`, the three `nanobot-*.ps1` files, `scripts/` tree (recursive), `README.md`, `SECURITY.md`, `AGENTS.md`, `CHANGELOG.md`, `LICENSE`, `data/.env.example`, `.github/`, `.gitattributes`, `.gitignore`.
...
- **`check_update.bat` is standalone.** It calls the GitHub API for `tertua/nanobot-usb` `lite` and compares against the `[sha]` section of `data\.lockhead` (gitignored). First run records a baseline; subsequent runs notify (one yellow line) or say OK. It does **not** auto-update nanobot itself — re-running `setup.bat` from a fresh release is the update path. Delete `data\.lockhead` to reset (lockhead is the single source of truth: setup-done sentinel + wrapper SHA; deleting it is a conscious reset).
...
- **`data\.env_key` is an optional stored passphrase** so the launcher can run non-interactively. Delete it to force an interactive passphrase prompt every run (per `SECURITY.md` recommendation). The LLM already has the API key via process env vars regardless; encryption's purpose is to keep plaintext out of the LLM's *context window*.
...
- **`tools.exec.pathAppend` in `data/config.json` is auto-set** by `post_config.py` to `bin`, `scripts`, `bin/git/cmd`, `bin/nodejs`, `bin/gh/bin`, `bin/Scripts` (insertion order). Don't hand-edit it after `setup.bat` — re-run `post_config.py` instead.
- **Default ports**: HTTP 8900, WS 8765 (read from `config.json` in `nanobot-gateway.ps1`; falls back to those values). `nanobot-gateway.ps1` silently kills stale processes on both ports at startup and registers an `PowerShell.Exiting` handler for cleanup on exit.
...
- `nanobot-setup.ps1` lines 162–166: comment says "Cleanup skipped — app/ and tmp/ preserved for inspection", but `$TMP_DIR` is actually removed (intentional — download/extract cache, not needed after install). `$APP_DIR` (the cloned/extracted nanobot source) genuinely stays on disk for debugging. Don't re-enable the `$APP_DIR` `Remove-Item` line unless you want a wiped source tree.
