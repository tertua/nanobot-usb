...
if (Test-Path $GhExe) {
    ...
    # gh release zip nests everything under a version-named top folder;
    # flatten it so $ROOT\bin\gh\bin\gh.exe is the final path.
    $nestedGh = Get-ChildItem -Path $GhDir -Directory | Where-Object { Test-Path (Join-Path $_.FullName "bin\gh.exe") } | Select-Object -First 1
    if ($nestedGh) {
        Get-ChildItem -Path $nestedGh.FullName -Recurse | Move-Item -Destination $GhDir -Force
        Remove-Item -Path $nestedGh.FullName -Recurse -Force
    }
    if (Test-Path $GhExe) {
