...
def detect_software(root: str) -> dict[str, str]:
    ...
    # pwsh (portable first)
    pwsh_exe = os.path.join(py_dir, "pwsh", "pwsh.exe")
    if not os.path.isfile(pwsh_exe):
        pwsh_exe = os.path.join(root, "bin", "pwsh7", "pwsh.exe")
    if not os.path.isfile(pwsh_exe):
        pwsh_exe = "pwsh.exe"
    ver = _run([pwsh_exe, "--version"])
    if ver:
        m = re.search(r"(\d+\.\d+\.\d+)", ver)
        if m:
            sw["pwsh"] = m.group(1)
...
def write_lockhead(root: str) -> None:
    ...
    for key in ("python", "nanobot", "git", "nodejs", "pwsh"):
